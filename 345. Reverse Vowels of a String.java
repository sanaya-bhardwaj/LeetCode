...
//Move the start pointer towards the end of the string until it points to a vowel character. 
//We do this by using the indexOf method to find the index of the character in the vowels string. If the character is not a vowel, we keep moving the start pointer.

//Similarly, move the end pointer towards the beginning of the string until it points to a vowel character. 
//Again, we use the indexOf method to check if the character is a vowel and keep moving the end pointer if it is not.
...
//The two pointers start and end move towards each other, and each character is processed once.

//Space complexity:
//The space complexity of this solution is O(n) because we create a character array word to store the characters of the input string. 
//The space used is proportional to the length of the input string. Other than that, we use a constant amount of extra space for the vowels string and a few integer variables, which doesn't depend on the input size.
